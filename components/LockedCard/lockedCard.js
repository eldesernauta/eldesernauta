...
const LockedCard = ({ bgColor, imgUrl, title, description, btnUrl }) => {
    ...
    return (
        <motion.div
            ...
            dragElastic={0.5}
            className={`${bgColor} locked transition-colors duration-500 w-full md:w-[500px] lg:w-[300px] flex flex-col justify-start items-center p-4 shadow-light border-2 border-light rounded-xl bg-[linear-gradient(to_right,#80808010_1px,transparent_1px),linear-gradient(to_bottom,#80808010_1px,transparent_1px)] bg-[size:50px_50px]`}
        >
            <Image
                src={imgUrl}
                height={100}
